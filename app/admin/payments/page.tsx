...
export default function AdminPaymentsPage() {
  ...
  const { data: pendingPaymentData, refetch: refetchPending } = useQuery({
    queryKey: ['admin', 'pending-payment'],
    queryFn: async () => {
      try {
  ...
  const cancelPaymentMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.setQueryData(['admin', 'pending-payment'], null);
  ...
  const checkStatusMutation = useMutation({
    ...
    onSuccess: (response) => {
      if (response.data.status === 'completed') {
        queryClient.invalidateQueries({ queryKey: ['admin', 'pending-payment'] });
  ...
  useEffect(() => {
    if (!pendingPaymentData?.data?.expiresAt) {
      setTimeRemaining(null);
      return;
    }

    const updateTimeRemaining = () => {
      const expiresAt = new Date(pendingPaymentData.data.expiresAt!).getTime();
      const now = Date.now();
                ...
                <div className="space-y-2 sm:space-y-3">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      className="h-11 sm:h-12 text-base sm:text-lg font-semibold pl-4 pr-14 sm:pr-12 placeholder:text-xs placeholder:sm:text-sm placeholder:font-normal"
